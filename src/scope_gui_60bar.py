...
class ScopeGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("WaveScope GUI (DT Pressure Sensor 0x03)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(conn, text="Detected socket (0x03):").grid(row=1, column=0, sticky="w", pady=(10, 0))
    ...
    def connect(self):
        ...
        try:
            ...
            pressure60 = self.vas.CheckPressure60Sensor()
            if pressure60["DT_1"]:
                self.detected_socket_var.set("DT_1")
            elif pressure60["DT_2"]:
    ...
    def _get_dt_socket_for_0x03(self) -> str | None:
        if self.vas is None:
            return None
        pressure60 = self.vas.CheckPressure60Sensor()
        if pressure60["DT_1"]:
    ...
    def start_stream(self):
        ...
        dt_socket = self._get_dt_socket_for_0x03()
        if dt_socket is None:
            messagebox.showerror("Sensor not found", "No sensor with ID 0x03 detected on DT_1/DT_2.")
            return
        ...
        self.ax.set_title(f"Scope A @ {dt_socket} / DRUCK_60 (0x03)")
